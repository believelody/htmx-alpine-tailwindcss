...
router.post('/login', (req, res, next) => utils.error.handleHttpError(req, res, next, async () => {
...
router.get('/check', async (req, res, next) => utils.error.handleHttpError(req, res, next, async () => {
  return res.render(`partials/auth/${req.query.component}`, { ...req.ctx, currentURLPathname: req.session.currentURLPathname });
}));

router.post('/logout', (req, res, next) => utils.error.handleHttpError(req, res, next, async () => {
